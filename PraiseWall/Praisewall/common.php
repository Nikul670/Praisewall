...
function wallShutdownHandler(){
    ...
    if (isset($fatals[$err['type']])) {
        $msg = 'ShutdownError ' . $_SERVER['UNIQUE_ID'] . ' :: ' . $fatals[$err['type']] . ': ' . $err['message'] . ' in ';
        $msg.= $err['file'] . ' on line ' . $err['line'];
        error_log($msg);
    }
}

register_shutdown_function('wallShutdownHandler');
...
$LIB = '';
// now added in php.ini set_include_path(get_include_path() .PATH_SEPARATOR . $LIB );
...
function praisewallautoload($classname) {
	...
	else if (($match = preg_match("/^Logger([\w]+)$/", trim($classname), $args)) != false) {
		$file = ShopbookLogger::Log4phpLoader(trim($args[0]));
	}
